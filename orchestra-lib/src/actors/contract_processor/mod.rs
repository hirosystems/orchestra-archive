use crate::datastore::contracts::{contract_db_read, contract_db_write, db_key, DBKey};
...
impl ContractProcessor {
    ...
    pub fn build_state(&mut self) {
        ...
        for contract_id in ordered_contracts_ids.into_iter() {
            ...
            full_analysis.insert(contract_id.clone(), (diagnostics, analysis, ast));
        }
        ...
        // Store artifacts
        {
            ...
            let full_analysis_bytes =
                serde_json::to_vec(&full_analysis).expect("Unable to serialize block");
            db.put(&self.db_key(DBKey::FullAnalysis), full_analysis_bytes)
                .unwrap();
            ...
            db.put(&self.db_key(DBKey::Interface), interface_bytes)
                .unwrap();
                    ...
                    for event_wrapper in tx.metadata.receipt.events.iter() {
                        match event_wrapper {
                            ...
                            StacksTransactionEvent::SmartContractEvent(event) => {
                                if event.contract_identifier == self.contract_id {
                                    custom_events
                                        .push((event.clone(), tx.transaction_identifier.clone()));
                                }
                            }
                    ...
                    for change in changes.iter() {
                        match change {
                            Changes::UpdateDataVar(var, new_value, txid) => {
                                db.put(
                                    &self.db_key(DBKey::Var(var)),
                                    hex_bytes(new_value).unwrap(),
                                )
                                .expect("Unable to write");
                            }
                            Changes::InsertDataMapEntry(map, (new_key, new_value), txid) => {
                                db.put(
                                    &self.db_key(DBKey::MapEntry(map, new_key)),
                                    hex_bytes(new_value).unwrap(),
                                )
                                .expect("Unable to write");
                            }
                            Changes::DeleteDataMapEntry(map, deleted_key, txid) => {
                                db.delete(&self.db_key(DBKey::MapEntry(map, deleted_key)))
                                    .expect("Unable to write");
                            }
                            Changes::UpdateDataMapEntry(map, (key, new_value), txid) => {
                                db.put(
                                    &self.db_key(DBKey::MapEntry(map, key)),
                                    hex_bytes(new_value).unwrap(),
                                )
                                .expect("Unable to write");
                            }
                            Changes::SendTokens(asset_id, (sender, value), txid) => {
                                let balance = match db
                                    .get(&self.db_key(DBKey::FT(asset_id, sender)))
                                {
                                    Ok(Some(value)) => {
                                ...
                                db.put(
                                    &self.db_key(DBKey::FT(asset_id, sender)),
                            ...
                            Changes::ReceiveTokens(asset_id, (recipient, value), txid) => {
                                let balance = match db
                                    .get(&self.db_key(DBKey::FT(asset_id, recipient)))
                                {
                                    Ok(Some(value)) => {
                                        u128::from_str_radix(&String::from_utf8(value).unwrap(), 10)
                                            .unwrap()
                                ...
                                db.put(
                                    &self.db_key(DBKey::FT(asset_id, recipient)),
                                    (balance + value).to_string(),
                                )
                                .expect("Unable to write");
                            }
                            Changes::SendNFT(asset_class_id, (asset_id, sender), txid) => {
                                db.delete(&self.db_key(DBKey::NFT(asset_class_id, asset_id)))
                                    .expect("Unable to write");
                            }
                            Changes::ReceiveNFT(asset_class_id, (asset_id, recipient), txid) => {
                                db.put(
                                    &self.db_key(DBKey::NFT(asset_class_id, asset_id)),
                                    recipient,
                                )
                                .expect("Unable to write");
                            }
